...
async def auto_actions(event):
    ...
    if str(event.sender_id) in enemies:
        ...
        # انتخاب تصادفی بدون تکرار به اندازه reply_count
        words_to_send = random.sample(bad_words, min(reply_count, len(bad_words)))

        # ساخت متن نهایی با جایگزینی املایی رندوم برای هر کلمه
        reply_text = "\n".join(typo_replace(word) for word in words_to_send)
